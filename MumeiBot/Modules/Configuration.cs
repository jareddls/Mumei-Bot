...
namespace MumeiBot.Modules
{
    public class Configuration : ModuleBase<SocketCommandContext>
    {
        ...
        public async Task Prefix(string prefix = null)
        {
            ...
            if (prefix.Length > 8)
            {
                await ReplyAsync("Length of new prefix is too long! (Up to 8 characters)");
                return;
            }
        ...
        //runmode async part is for allowing commands to asynchronously on aseparate process
        //for database related processes
        [Command("ranks", RunMode = RunMode.Async)]
        public async Task Ranks()
        {
            var ranks = await _serverHelper.GetRanksAsync(Context.Guild);
            //if server has no ranks
            if(ranks.Count == 0)
            {
            ...
            //triggertypingasync shows that the bot is typing
        ...
        //remainder means it allows spaces in strings
        public async Task AddRank([Remainder] string name)
        {
            await Context.Channel.TriggerTypingAsync();
            var ranks = await _serverHelper.GetRanksAsync(Context.Guild);

            //search for role passed in string name 
            //case insensitive
            //compares and lets us know if its true, but ignores case sensitivity
        ...
        [Command("welcome")]
        [RequireUserPermission(GuildPermission.Administrator)]
        public async Task Welcome(string option = null, string value = null) //= null makes it optional
        {
        ...
        [Command("logs")]
        [RequireUserPermission(GuildPermission.Administrator)]
        public async Task Logs(string value = null) //= null makes it optional
        {
            if (value == null)
            {
